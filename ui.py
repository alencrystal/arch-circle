...
def draw_level_up_menu(screen, buffs, selected_option):
    ...
    for i, buff in enumerate(buffs):
        ...
        # Rimuovi le scritte di default sui pulsanti
        ...
        # Aggiungi descrizione dell'effetto del potenziamento
        if buffs[i]["name"] == "Heal":
            effect_text = "heal"
        elif buffs[i]["name"] == "Max HP":
            effect_text = "+HP"
        elif buffs[i]["name"] == "Faster Reload":
            effect_text = "+atk speed"
        elif buffs[i]["name"] == "Move Speed":
            effect_text = "+move speed"
        elif buffs[i]["name"] == "Extra Score":
            effect_text = "+score"
        effect_rendered = FONT_SMALL.render(effect_text, True, WHITE)
        effect_rect = effect_rendered.get_rect(center=rect.center)
        screen.blit(effect_rendered, effect_rect)
